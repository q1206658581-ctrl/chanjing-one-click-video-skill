...
1. 已配置蝉镜凭证（`~/.chanjing/credentials.json` 包含 `app_id` 和 `secret_key`）
2. 已设置 `DEERAPI_API_KEY` 环境变量（小鹿 DeerAPI，用于 LLM 生成文案/分镜）
3. 已设置 `CHAN_SKILLS_DIR` 指向 chan-skills 仓库根目录（渲染必填）
